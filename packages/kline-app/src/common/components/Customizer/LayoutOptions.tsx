...
import { Popover, Tooltip, Switch, Slider, Radio } from "antd";
...
class LayoutOptions extends React.Component<any> {
  onLayoutChange = e => {
  ...
  render() {
    ...
    return (
      <section>
        ...
        <div className="row m-0 layout-options clearfix">
          <RadioGroup onChange={this.onLayoutChange}>
            <RadioButton value="1" checked={layout === "1"}>
              <Tooltip title="Default App layout">
                <span className="layout-opition">1</span>
              </Tooltip>
            </RadioButton>
            <RadioButton value="2" checked={layout === "2"}>
              <Tooltip title="App v2 layout">
                <span className="layout-opition">2</span>
              </Tooltip>
            </RadioButton>
            <RadioButton value="3" checked={layout === "3"}>
              <Tooltip title="Header-Content-Footer layout">
                <span className="layout-opition">3</span>
              </Tooltip>
            </RadioButton>
            <RadioButton value="4" checked={layout === "4"}>
              <Tooltip title="Content Only (Fullscreen page)">
                <span className="layout-opition">4</span>
              </Tooltip>
            </RadioButton>
            <RadioButton value="custom">
              <Popover
                content="Building custom layouts is quite easy!"
                title="Your Own"
                trigger="hover"
              >
                <span className="layout-opition">Custom</span>
              </Popover>
            </RadioButton>
          </RadioGroup>
        </div>
        ...
        <div className="layout-setting-item">
          <Tooltip title='In layout "1", Fixed Header can only be activated when Fixed Sidenav is activated'>
            <span className="text-dotted">
              Fixed Header <ExclamationCircleFilled />
            </span>
          </Tooltip>
          <Switch
