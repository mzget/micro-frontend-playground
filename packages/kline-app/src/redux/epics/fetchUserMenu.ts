...
import { AppStateType, ActionType } from 'app/common/types';
...
import {
    ...
    LOGIN_FINISH,
} from 'app/features/auth/reducers/authReducer';

import {
    getUserMenu,
    getUserMenuFinish,
} from 'app/redux/reducers/permission/permissionReducer';

export const fetchUserMenuEpic = (action$, state$) =>
    action$.pipe(
        filter(
            (action: ActionType) =>
                action.type === VERIFY_SESSION_FINISH ||
                action.type === LOGIN_FINISH
        ),
        map(getUserMenu),
        mergeMap(action => {
