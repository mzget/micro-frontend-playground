...
import SelectRC from "common/components/Form/SelectRC";
...
import { AppStateType, MockData } from "common/types";
                ...
                <Link
                  to={`${
                    InstantRedemtion.KLine.filter(
                      (v) => v.name === "RE_KLINE_CREATE_CAMPAIGN"
                    )[0].path
                  }`}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Row>
                ...
                <Controller
                  as={
                    <Input
                      disabled={!editable}
                      maxLength={150}
                      placeholder={Locale.KLINE.LABEL.CAMPAIGN}
                    />
                  }
                  name="campaign"
                  control={control}
                ...
                <Controller
                  as={
                    <Input
                      disabled={!editable}
                      maxLength={9}
                      placeholder={Locale.KLINE.LABEL.CAMPAIGN_CODE}
                    />
                  }
                  name="promo_code"
                  control={control}
            ...
            <Col span={8}>
              <Form.Item
                label={
                  <FormItemLabel label={Locale.KLINE.LABEL.CAMPAIGN_TYPE} />
                }
              >
                <CampaignType
                  hookForm={method}
          ...
          <Row>
            <Col span={7}>
              <Form.Item
                label={
                  <FormItemLabel label={Locale.KLINE.LABEL.CAMPAIGN_CHANNEL} />
                }
                extra={<ErrorMessage errors={errors} name="campaign_channel" />}
              >
                <PaymentChannel
                  hookForm={method}
            ...
            <Col span={7}>
              <Form.Item
                label={Locale.KLINE.CAMPAIGN_INQUIRY.CAMPAIGN_START_DATE}
              ...
              >
                <DatePicker
                  ...
                  onChange={(...args) => {
                    ...
                    setValue("start_date", args[0]?.startOf("day") as any);
                    return dateString;
                  }}
            ...
            <Col span={8}>
              <Form.Item
                label={Locale.KLINE.CAMPAIGN_INQUIRY.CAMPAIGN_END_DATE}
                extra={<ErrorMessage errors={errors} name="end_date" />}
              >
                <DatePicker
                  disabled={!editable}
                  ...
                  onChange={(...args) => {
                    let dateString: any = args[1];
                    setValue("end_date", args[0]?.endOf("day") as any);
                    return dateString;
                  }}
          ...
          <Section label={"Search Transactions Result"}>
            <EnhancedCSV
              fileName={`Campaign_List_${moment().format(EXPORT_DATE_FORMAT)}`}
